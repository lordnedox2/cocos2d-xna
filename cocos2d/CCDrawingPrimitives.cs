...
namespace Cocos2D
{
    public partial class CCDrawingPrimitives
    {
        ...
        {
            m_Batch.End();
            //MARCO Reset my props
            m_Batch.SetTexture(null);
        }
        ...
        {
            DrawCircle(center, radius, b2Vec2.Zero, 20, true, color);
        }
        ...
        public static void DrawCircle(b2Vec2 center, float radius, b2Vec2 axis, int segments, bool drawLineToCenter, b2Color color)
        {
            ...
            if (drawLineToCenter)
            {
                DrawLine(center, new b2Vec2(center.x + (axis.x * radius), center.y + (axis.y * radius)), color);
            }
        }
        ...
        public static void DrawTexturedPoly(CCPoint[] vertices, CCPoint[] textureCoords, int count)
        {
            var colorFill = new Color(255, 255, 255, 255);
            for (int i = 0; i < count; i++)
            {
                m_Batch.AddVertex(new Vector2(vertices[i].X, vertices[i].Y), new Vector2(textureCoords[i].X, textureCoords[i].Y), PrimitiveType.TriangleStrip);
            }
        }
    }
